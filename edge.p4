                  ...
                  inout standard_metadata_t standard_metadata)
{
    ...
    action assign_switch_action(bit<7> switch_id) {
        hdr.cust.setValid();
        hdr.cust.dest_switch = switch_id;
        hdr.cust.ethertype = hdr.ethernet.ether_type;
        hdr.ethernet.ether_type = 0xFFFF;
        hdr.cust.tenant = standard_metadata.ingress_port;
    }

    action decap_action(bit<9> port) {
        hdr.ethernet.ether_type = hdr.cust.ethertype;
        standard_metadata.egress_spec = port;
        hdr.cust.setInvalid();
    }
    ...
    table switch_select {
        ...
        actions = {
            drop_action;
            assign_switch_action;
        }
        size = 1024;
        default_action = drop_action;
    }

    table endpoint_match {
        key = {
            meta.destination_addr: exact;
